...
import { KeycloakEvent, KeycloakEventType } from '../interfaces/keycloak-event.interface'
import {
	ExcludedUrl,
	ExcludedUrlRegex,
	KeycloakOptions,
} from '../interfaces/keycloak-options.interface'
...
export class KeycloakService {
	 ...
	 */
	public init(options: KeycloakOptions = {}, authentication?: boolean): Promise<boolean> {
		return new Promise((resolve, reject) => {
			...
			this._instance
				...
				.then(async (authenticated) => {
					if (authentication && authenticated && this._loadUserProfileAtStartUp) {
						await this.loadUserProfile()
					}
					resolve(authentication ? authenticated : true)
				})
				.catch((kcError) => {
	...
	public register(
		...
		return new Promise((resolve, reject) => {
			this._instance
				...
				.catch(() => reject('An error happened during the register execution.'))
		})
	}
	...
	public getUserRoles(allRoles: boolean = true): string[] {
		...
		if (this._instance.resourceAccess) {
			for (const key in this._instance.resourceAccess) {
				if (key in this._instance.resourceAccess) {
	 ...
	 */
	public async loadUserProfile(forceReload: boolean = false): Promise<Keycloak.KeycloakProfile> {
		return new Promise((resolve, reject) => {
	 ...
	 */
	public async getToken(forceLogin = true): Promise<string> {
		try {
	...
	public getUsername(): string {
		...
		return this._userProfile?.username ?? null
	}
